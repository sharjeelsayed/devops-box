...
Vagrant.configure("2") do |config|
	...
	config.vm.define "devops-box" do |devops-box|
		devops-box.vm.box = BOX_IMAGE
		devops-box.vm.provider "virtualbox" do |v|
			v.name = "devops-box"
			v.memory = MEMORY
    		v.cpus = CPU
		end
		devops-box.vm.provision "shell", path: "scripts/install.sh"
		devops-box.vm.provision "shell", inline: <<-SHELL
	...
	config.vm.define "devops-box-additional" do |devops-box|
		devops-box-additional.vm.box = BOX_IMAGE
		devops-box-additional.vm.provider "virtualbox" do |v|
			v.name = "devops-box-additional"
			v.memory = MEMORY
    		v.cpus = CPU
		end
		devops-box-additional.vm.provision "shell", path: "scripts/install.sh"
		devops-box-additional.vm.provision "shell", inline: <<-SHELL
